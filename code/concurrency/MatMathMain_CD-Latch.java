...
class MatMathImpl implements MatMath
{
	...
	ExecutorService executor = 
		Executors.newFixedThreadPool(A.length*B[0].length);
    	for (int r=0; r<A.length; r++)
    	    for (int c=0; c<B[0].length; c++)
    	    { executor.execute(new 
    	    	RowColProdExecutable(A, B, C, r, c, A[r].length, latch));}
    	executor.shutdown();
    	try { latch.await(); }
	...
	ExecutorService executor = 
		Executors.newFixedThreadPool(A.length*A[0].length);
    	for (int r=0; r<A.length; r++)
    	    for (int c=0; c<B[0].length; c++)
    	    { executor.execute(new RowColSumExecutable(A, B, C, r, c, latch)); }
    	executor.shutdown();
    	try { latch.await(); }
    	catch (InterruptedException e) {e.printStackTrace();}
    	}
